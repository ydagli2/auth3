...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (!epic.owners.includes(interaction.user.id) && !data) {
      return interaction.reply({ embeds: [new discord.MessageEmbed().setDescription(`You're Not Whitelisted :x: !`).setColor('RED')] })
    }

    if (command.ownerOnly) {
      if (!epic.owners.includes(interaction.user.id)) {
        return interaction.reply({ embeds: [new discord.MessageEmbed().setDescription(`You're Not The Owner Of The Bot ( If You Are And Got Banned Join discord.gg/bxz To Transfer Owner ) :x:`).setColor('RED')] })
      }
    }
